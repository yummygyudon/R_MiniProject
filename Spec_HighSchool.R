...
sum(is.na(Spec_h$TOEIC))# 토익 없는 경우
table(h_TOEIC) # 있는 경우 분포
...
h_T.Speaking <- factor(Spec_h$T.Speaking, levels = tos_lv)
length(h_T.Speaking)
sum(is.na(h_T.Speaking))# 토스 없는 경우
table(h_T.Speaking) # 있는 경우 분포



#오픽
Spec_h$OPIC <- gsub("[-]","",Spec_h$OPIC)
h_OPIC <- factor(Spec_h$OPIC, levels = opic_lv)
length(h_OPIC) # 총 경우
sum(is.na(h_OPIC))# 오픽 없는 경우
